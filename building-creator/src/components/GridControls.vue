  ...
  <div class="controls">
    ...
    <div class="grids-container" ref="gridsContainer">
      <div class="grids" v-for="(grid, index) in props.grids" :key="grid.id" ref="grids-ref">
        ...
        <div class="controls-expandable-icon" @click="changeExpand(index)">
          <v-icon v-if="isgridExpanded[index]" name="bi-arrow-up" style="fill: black" />
        ...
        <div class="controls-expandable" :class="{ expanded: isgridExpanded[index] }">
          ...
          <div class="controls--group">
            <label :for="`number-of-windows-${grid.id}`"
              >Number of columns: {{ grid.columns }}</label
            >
            <input
              :id="`number-of-windows-${grid.id}`"
              type="range"
          ...
          <div class="controls--group">
            ...
            >
              <div
                v-for="index in Array.from({ length: grid.rows * grid.columns }, (_, i) => i + 1)"
                :key="index"
              >
                <input
          ...
          <div class="controls--group">
            ...
            <input
              ...
              @input="
                updateGrid(grid.id, 'windowWidth', `${($event.target as HTMLInputElement).value}%`)
              "
            />
          </div>
          <div class="controls--group">
            ...
            <input
              ...
              @input="
                updateGrid(grid.id, 'windowHeight', `${($event.target as HTMLInputElement).value}%`)
              "
            />
          </div>
          <div class="controls--group">
            ...
            <input
              ...
              @input="
                updateGrid(grid.id, 'gridWidth', `${($event.target as HTMLInputElement).value}%`)
              "
            />
          </div>
          <div class="controls--group">
            ...
            <input
              ...
              @input="
                updateGrid(grid.id, 'gridHeight', `${($event.target as HTMLInputElement).value}%`)
              "
            />
          </div>
          <div class="controls--group">
            ...
            <input
              ...
              @input="
                updateGrid(grid.id, 'rowGap', `${($event.target as HTMLInputElement).value}px`)
              "
            />
          </div>
          <div class="controls--group">
            ...
            <input
              ...
              @input="
                updateGrid(grid.id, 'columnGap', `${($event.target as HTMLInputElement).value}px`)
              "
            />
          </div>
          <div class="controls--group">
            ...
            <input
              ...
              @input="
                updateGrid(grid.id, 'borderRadius', `${($event.target as HTMLInputElement).value}%`)
              "
            />
          </div>
          <div class="controls--group">
            ...
            <div
              v-for="(color, colorIndex) in grid.colors"
            ...
            >
              ...
              <div class="d-flex">
                <v-icon
                  v-if="grid.colors.length > 1"
                  name="md-deleteforever-outlined"
                  style="position: absolute; right: 0; top: 0; cursor: pointer"
                  @click="deleteColor(grid, colorIndex)"
                ></v-icon>
                <input
                  :id="`color-${grid.id}`"
                  type="color"
              ...
              <div class="d-flex">
                <label :for="`color-percentage-${grid.id}-${index}`"
                  >{{ parseInt(color.percentage) }}%</label
                >
                <input
                  :id="`color-percentage-${grid.id}-${index}`"
                  type="range"
                  min="1"
                  max="100"
            ...
            <button class="grid-button" @click="addColors(grid)" style="background: blue">
          ...
          <div class="controls--group">
            ...
            <label :for="`border-bottom-${grid.id}`"
              >Border-bottom: {{ grid.borderBottom?.size }}</label
            >
            <div class="d-flex">
              <input
                type="range"
            ...
            <label :for="`border-bottom-${grid.id}`"
              >Border-left: {{ grid.borderLeft?.size }}</label
            >
            <div class="d-flex">
              <input
                type="range"
            ...
            <label :for="`border-bottom-${grid.id}`"
              >Border-right: {{ grid.borderRight?.size }}</label
            >
            <div class="d-flex">
              <input
                type="range"
...
import { nextTick, ref, useTemplateRef, watch, onMounted } from 'vue'
...
watch(
  ...
  async (value: Grid | undefined, lastValue: Grid | undefined) => {
    ...
    // Check if gridsRef.value is populated
    if (!gridsRef.value || !gridsRef.value[selectedgridIndex]) {
      console.log('gridsRef not ready yet:', gridsRef.value)
      return
    }
    ...
    await nextTick() // Ensure DOM updates after expanding
    scrollToTarget(selectedgridRef)
    await nextTick() // Ensure DOM updates before adding class
...
const switchPosition = (index: number, flag: number) => {
  ...
  const currentgrid = Object.assign(props.grids[index])
  const gridToSwitch = Object.assign(props.grids[switchIndex])
...
const duplicateGrid = (grid: Grid) => {
  ...
  emitGridChanges([...props.grids, deepCloneGrid])
}
...
const emitGridChanges = (newGrid: Grid[]) => {
  emit('update:grids', newGrid, props.cube.id)
}
</script>
